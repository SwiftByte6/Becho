...
export default function MarketplaceClient() {
  ...
  const handleRequestMaterial = async (listing) => {
    ...
    try {
      ...
      if (!numericQuantity || numericQuantity <= 0 || numericQuantity > Number(listing.quantity)) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        />
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="px-4 py-2.5 border border-[#dccfb9] rounded-xl text-sm bg-[#fcf8f1] text-[#6f655a] outline-none focus:border-[#7f925b] transition-all">
        ...
        <Button variant="primary" size="md" onClick={() => loadListings(search, category)}>Search</Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {items.map((listing) => (
          <div key={listing.id} className="bg-[#fcf8f1] rounded-2xl border border-[#dccfb9] shadow-[0_10px_30px_rgba(95,79,64,0.08)] p-5 flex flex-col gap-3 hover:-translate-y-0.5 hover:shadow-[0_18px_40px_rgba(95,79,64,0.12)] transition-all duration-200">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-[#4e4033]">{listing.material_name}</p>
                <p className="text-[#9a8f82] text-xs mt-0.5">{listing.users?.company_name || 'Unknown company'}</p>
              </div>
              <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${Number(listing.price) === 0 ? 'bg-[#edf4eb] text-[#5f6f43]' : 'bg-[#ece9f6] text-[#64568a]'}`}>
                {Number(listing.price) === 0 ? 'Free' : `₹${listing.price}/${listing.unit}`}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#efe4d4] text-[#7a7065] text-xs">
                <Calendar size={11} /> {new Date(listing.created_at).toLocaleDateString()}
              </span>
            </div>

            <p className="text-sm font-semibold text-[#6f655a]">{listing.quantity} {listing.unit} available</p>

            <Button variant="primary" size="sm" fullWidth onClick={() => handleRequestMaterial(listing)}>
