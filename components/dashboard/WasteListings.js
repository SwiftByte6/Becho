  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {displayed.map(
          ({ id, name, quantity, category, status, date, description }) => (
            <div
              key={id}
              className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3 hover:-translate-y-0.5 hover:shadow-md transition-all duration-200"
            >
              {/* Header */}
              ...
              <p className="text-slate-400 text-xs leading-relaxed border-t border-slate-50 pt-2">
                {description}
              </p>
